...
xOmega,xAmplitude,yOmega,yAmplitude = 2*math.pi, 25, 2*math.pi, 25
#define the x and y offset. use math.pi/2 for cosine 
xOffset, yOffset =0,0#math.pi/2,0
...
plt.title("Wavefront Diagram of a Single Point Sound Source with Equations: \n x="+str(round(xAmplitude,3))+"sin("+str(round(xOmega,3))+'t+'+str(round(xOffset,3))+"), y="+str(round(yAmplitude,3))+"sin("+str(round(yOmega,3))+'t+'+str(round(yOffset,3))+")\n at Time: t="+str(round(timeStep*(stepNumber),3)))
plt.xlabel("x (m)") 
plt.ylabel("y (m)") 
...
for j in range(len(area[0])):
    ...
    for k in range(len(area[0][0])):  
        ...
        for i in range(stepNumber):
            ...
            def equations(p):
                x, y = p
                return (x+y-tM, x-((((xAmplitude*math.sin(xOmega*y+xOffset))-xM)**2+(yAmplitude*math.sin((yOmega*y+yOffset))-yM)**2)**.5)/SPEEDOFSOUND)
