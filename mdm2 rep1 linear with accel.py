...
dist = 2
#define the x and y equation's m & b (in the form x=xEM*t+xEB etc)
xEM, xEB, yEM, yEB = 0, -50, 0,-50
#define the acceleration of x and y (will turn into .5*xEA*t^2 etc)
xEA,yEA = SPEEDOFSOUND,2.5*SPEEDOFSOUND
...
plt.title("Wavefront Diagram of a Single Point Sound Source with Equations: \n x="+str(round(xEA/2,3))+"t^2+"+str(round(xEM,3))+"t+"+str(round(xEB,3))+", y="+str(round(yEA/2,3))+"t^2+"+str(round(yEM,3))+"t+"+str(round(yEB,3))+"\n at Time: t="+str(round(timeStep*(stepNumber),3)))
plt.xlabel("x (m)") 
plt.ylabel("y (m)") 
...
for i in range(numCircles):
    circTime = i/f
    circlesList.append([circTime,(xEA/2)*circTime**2+circTime*xEM+xEB,(yEA/2)*circTime**2+circTime*yEM+yEB])
    c = plt.Circle(((xEA/2)*circTime**2+circTime*xEM+xEB,(yEA/2)*circTime**2+circTime*yEM+yEB), radius =(SPEEDOFSOUND*(timeStep*stepNumber-circTime)), fill = False)
    plt.gca().add_artist(c)
print(circlesList)
imgData = str(timePeriod) + ',' + str(timeStep) + ',' + str(f) + ',' + str(areaHeight) + ',' + str(areaWidth) + ',' + str(dist) + ',' +  str(xEA) + ',' +str(xEM) + ',' + str(xEB) + ',' +str(yEA) + ',' + str(yEM) + ',' + str(yEB)
...
for j in range(len(area[0])):
    ...
    for k in range(len(area[0][0])):  
        ...
        for i in range(stepNumber):
            ...
            def equations(p):
                x, y = p
                return (x+y-tM, x-(((((xEA/2)*y**2+xEM*y+xEB)-xM)**2+(((yEA/2)*y**2+yEM*y+yEB)-yM)**2)**.5)/340,)
...
write1 = "x = "+str(xEA) + 't^2+' +str(xEM)+"t + "+str(xEB)
write2 = "y = "+str(yEA) + 't^2+' +str(yEM)+"t + "+str(yEB)
