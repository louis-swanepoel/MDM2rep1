...
yOmega,yAmplitude, = 2*math.pi, 50
...
for i in range(numCircles):
    circTime = i/f
    circlesList.append([circTime,xEM*circTime+xEB,yAmplitude*math.sin(yOmega*circTime+yOffset)])
    c = plt.Circle(((xEM*circTime+xEB),yAmplitude*math.sin(yOmega*circTime+yOffset)), radius =(SPEEDOFSOUND*(timeStep*stepNumber-circTime)), fill = False)
    plt.gca().add_artist(c)
print(circlesList)
imgData = str(timePeriod) + ',' + str(timeStep) + ',' + str(f) + ',' + str(areaHeight) + ',' + str(areaWidth) + ',' + str(dist) + ',' +  str(xEM) + ',' + str(xEB) + ','+str(yAmplitude) +','+ str(yOmega) + ',' + str(yOffset)+'circ'
...
for j in range(len(area[0])):
    ...
    for k in range(len(area[0][0])):  
        ...
        for i in range(stepNumber):
            ...
            def equations(p):
                x, y = p
                return (x+y-tM, x-((((xEM*y+xEB)-xM)**2+(yAmplitude*math.sin((yOmega*y+yOffset))-yM)**2)**.5)/SPEEDOFSOUND)
...
write1 = "x = "+str(round(xEM,3))+"t+"+str(round(xEB,3))
